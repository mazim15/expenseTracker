...
`;
  ...
  async predictCategorySpending(
    ...
    const last30Days = categoryExpenses.filter(
      (e) => new Date(e.date) >= new Date(Date.now() - 30 * 24 * 60 * 60 * 1000),
    );
    const last60Days = categoryExpenses.filter(
  ...
  private prepareHistoricalData(expenses: ExpenseType[], timeframe: string): string {
    ...
    expenses.forEach((expense) => {
      ...
      monthlyData[monthKey][expense.category] =
        (monthlyData[monthKey][expense.category] || 0) + expense.amount;
    });
...
- Average monthly spending: $${(
      recentMonths.reduce(
        (sum, month) => sum + Object.values(monthlyData[month]).reduce((s, a) => s + a, 0),
        0,
      ) / recentMonths.length
    ).toFixed(2)}
- Growth trend: ${this.calculateGrowthTrend(
...
${weeklyTotals
  .reverse()
  .map((total, index) => `Week ${index + 1}: $${total.toFixed(2)}`)
  .join("\n")}

Recent Transactions:
${last90Days
  .slice(-10)
  .map((e) => `${e.date.toLocaleDateString()}: $${e.amount.toFixed(2)} - ${e.description}`)
  .join("\n")}
`;
  }
...
`;
  ...
  private getFallbackForecast(
    ...
    const budgetRisk = currentBudgets
