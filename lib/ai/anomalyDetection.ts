...
export class AnomalyDetectionService {
  ...
  async detectAnomalies(expenses: ExpenseType[], lookbackDays = 30): Promise<SpendingAnomaly[]> {
    ...
    return anomalies.filter(
      (anomaly, index, self) => index === self.findIndex((a) => a.id === anomaly.id),
    );
  }
  ...
  private detectAmountAnomalies(
    ...
    recentExpenses.forEach((expense) => {
      ...
      const standardDeviation = Math.sqrt(
        stats.amounts.reduce((sum, amount) => sum + Math.pow(amount - stats.average, 2), 0) /
          stats.count,
      );
  ...
  private detectUnusualCategories(
    recentExpenses: ExpenseType[],
    allExpenses: ExpenseType[],
  ): SpendingAnomaly[] {
    const anomalies: SpendingAnomaly[] = [];
    ...
    const categoryUsage = allExpenses.reduce(
      (acc, expense) => {
...
`;
  ...
  private prepareExpenseDataForAI(
    recentExpenses: ExpenseType[],
    allExpenses: ExpenseType[],
  ): string {
