...
export function useLogger() {
  ...
  const logActionError = useCallback(
    async (error: Error, context?: string, details?: Record<string, any>) => {
      try {
  ...
  const logActionPerformance = useCallback(
    async (action: string, duration: number, details?: Record<string, any>) => {
      try {
  ...
  const withPerformanceLogging = useCallback(
    <T extends any[], R>(
      actionName: string,
  ...
  const withActionLogging = useCallback(
    <T extends any[], R>(
      actionName: string,
      fn: (...args: T) => Promise<R> | R,
      additionalDetails?: Record<string, any>,
